...
class contact extends krn_abstract{	
	...
	function GetResult() {
		...
		if ($contact['Address']) $contactsContent .= '<h4>Адрес</h4><p>' . $contact['Address'] . '</p>';
		if ($contact['Tel1'] || $contact['Tel2']) $contactsContent .= '<h4>Телефон</h4><p id="tel">' . ($contact['Tel1'] ? '<a href="tel:'.preg_replace('/[^\d\+]/', '', $contact['Tel1']).'">'.$contact['Tel1'].'</a><br>' : '') . ($contact['Tel2'] ? '<a href="tel:'.preg_replace('/[^\d\+]/', '', $contact['Tel2']).'">'.$contact['Tel2'].'</a>' : '') . '</p>';
